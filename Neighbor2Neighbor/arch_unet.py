...
class UpsampleCat(nn.Module):
    def __init__(self, in_nc, out_nc):
        ...
        self.deconv = nn.ConvTranspose2d(in_nc, out_nc, 2, 2, 0, bias=False, device=self.device)
...
class UNet(nn.Module):
    def __init__(self,
        ...
        # Encoder part
        self.enc_conv0 = nn.Conv2d(self.in_nc, self.n_feature, 3, 1, 1, device = self.device)
        self.enc_conv1 = nn.Conv2d(self.n_feature, self.n_feature, 3, 1, 1, device = self.device)
        initialize_weights(self.enc_conv0, 0.1)
        initialize_weights(self.enc_conv1, 0.1)
        self.pool1 = nn.MaxPool2d(2)

        self.enc_conv2 = nn.Conv2d(self.n_feature, self.n_feature, 3, 1, 1, device = self.device)
        initialize_weights(self.enc_conv2, 0.1)
        self.pool2 = nn.MaxPool2d(2)

        self.enc_conv3 = nn.Conv2d(self.n_feature, self.n_feature, 3, 1, 1, device = self.device)
        initialize_weights(self.enc_conv3, 0.1)
        self.pool3 = nn.MaxPool2d(2)

        self.enc_conv4 = nn.Conv2d(self.n_feature, self.n_feature, 3, 1, 1, device = self.device)
        initialize_weights(self.enc_conv4, 0.1)
        self.pool4 = nn.MaxPool2d(2)

        self.enc_conv5 = nn.Conv2d(self.n_feature, self.n_feature, 3, 1, 1, device = self.device)
        initialize_weights(self.enc_conv5, 0.1)
        self.pool5 = nn.MaxPool2d(2)

        self.enc_conv6 = nn.Conv2d(self.n_feature, self.n_feature, 3, 1, 1, device = self.device)
        ...
        self.dec_conv1b = nn.Conv2d(96, 96, 3, 1, 1, device = self.device)
        initialize_weights(self.dec_conv1b, 0.1)
        if blindspot:
            self.nin_a = nn.Conv2d(96 * 4, 96 * 4, 1, 1, 0, device = self.device)
            self.nin_b = nn.Conv2d(96 * 4, 96, 1, 1, 0, device = self.device)
        else:
            self.nin_a = nn.Conv2d(96, 96, 1, 1, 0, device = self.device)
            self.nin_b = nn.Conv2d(96, 96, 1, 1, 0, device = self.device)
        initialize_weights(self.nin_a, 0.1)
        initialize_weights(self.nin_b, 0.1)
        self.nin_c = nn.Conv2d(96, self.out_nc, 1, 1, 0, device = self.device)
