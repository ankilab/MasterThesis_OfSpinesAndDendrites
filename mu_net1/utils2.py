...
def predict_patch(model, img, recon_img, occupancy, patch_sz, max_value, idx, level_idx=0):
    (z, y, x) = idx
...
def window_sliding(self, img, sampling_step, patch_sz, max_value, batch_sz=1, n_levels=1):
    img_sz = img.shape
    ...
    for z in range(0, img_sz[0] - patch_sz[0] + 1, sampling_step[0]):
        for y in range(0, img_sz[1] - patch_sz[1] + 1, sampling_step[1]):
            for x in range(0, img_sz[2] - patch_sz[2] + 1, sampling_step[2]):
                recon_img, occupancy = predict_patch(self.model, img, recon_img, occupancy, patch_sz, max_value,
                                                     (z, y, x), level_idx)
